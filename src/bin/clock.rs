...
fn main() {
    ...
    let transform = Transform::new().rotate_z(std::f64::consts::PI / 6.);
    let mut point = point3(0.0, 0.8, 0.0);

    for _ in 0..12 {
        point = transform.local_to_world * point;
        let x = (canvas.width as f64 / 2. + point.x * canvas.width as f64 / 2.).round() as usize;
        let y = (canvas.height as f64 / 2. - point.y * canvas.height as f64 / 2.).round() as usize;
